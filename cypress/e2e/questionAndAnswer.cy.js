...
const filePath = 'cypress/fixtures/Sonuclar.docx';
    ...
    it('Boss sorulan sorulara beklenen cevapları verir', () => {
        ...
        const questions = [
            ...
            },
            {
                ...
                expectedNames: ["MUS03", "MUS30", "MUS13", "MUS02", "MUS27", "MUS23", "MUS01", "MUS05", "MUS18", "MUS14", "MUS17", "MUS11", "MUS07", "MUS24", "MUS25", "MUS16", "MUS22", "MUS19", "MUS12", "MUS21", "MUS20"],
                counter: 85
            },
            {
        ...
        function getValidResponse(selector, retries = 5) {
            return new Cypress.Promise((resolve, reject) => {
                function checkResponse(attempt) {
                    cy.get(selector).invoke('text').then((text) => {
                        cy.log(`Deneme ${attempt}: `, text);

                        if (text.includes("BOSS dosyaları inceliyor...") && attempt < retries) {
                            cy.wait(4000);
                            checkResponse(attempt + 1);
                        } else {
                            resolve(text.trim());
                        }
                    });
        ...
        questions.forEach(({ question, expectedNames, counter }) => {
            ...
            getValidResponse(`body > div:nth-child(1) > div:nth-child(1) > main:nth-child(3) > div:nth-child(1) > div:nth-child(1) > div:nth-child(2) > div:nth-child(2) > div:nth-child(1) > div:nth-child(${counter}) > div:nth-child(1) > div:nth-child(2) > div:nth-child(1)`)
                .then((text) => {
                    cy.log(text);
                    ...
                    // Tüm sorular tamamlandığında dosyaya yaz
                    if (results.length === questions.length) {
                        ...
                        cy.task("saveResultsAsDocx", results);
